...
export class KRScraperStrategy implements IScraperStrategy {
    supportedRegions = ['KR'];
    ...
    async search(query: string): Promise<Product[]> {
        ...
        // 1. Get KR merchants
        const merchants = MerchantRegistry.getByRegion('KR');
     ...
     */
    private filterSuspiciousProducts(products: Product[]): Product[] {
        ...
        return products.filter(product => {
            ...
            if (MOCK_INDICATORS.some(kw => title.includes(kw) || id.includes(kw))) return false;
            if (id.includes('_fallback_') || id.includes('fallback')) return false;

            // Suspiciously low price check
            if (avgPrice > 0 && price < avgPrice * 0.4) return false;

            if (SUSPICIOUS_KEYWORDS.some(kw => title.includes(kw.toLowerCase()))) return false;
