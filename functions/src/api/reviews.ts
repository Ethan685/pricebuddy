...
const KEYWORDS: Record<string, { topic: string, sentiment: 'positive' | 'negative' }> = {
...
function analyzeHeuristically(reviews: string[]) {
    const clusters: Record<string, { pos: number, neg: number, count: number }> = {};
    let spamCount = 0;
    ...
    for (const text of reviews) {
        ...
        for (const [key, meta] of Object.entries(KEYWORDS)) {
            if (lower.includes(key)) {
                ...
                if (!clusters[meta.topic]) clusters[meta.topic] = { pos: 0, neg: 0, count: 0 };
                if (sentiment === 'positive') clusters[meta.topic].pos++;
                else clusters[meta.topic].neg++;
                clusters[meta.topic].count++;
            }
        }
    }

    const clusterArray: Cluster[] = Object.entries(clusters).map(([topic, stats]) => {
        const total = stats.pos + stats.neg;
        const ratio = total > 0 ? stats.pos / total : 0;
        let sentiment: Cluster['sentiment'] = 'neutral';
    ...
    if (topCluster) {
        if (topCluster.sentiment === 'positive') summary = `Most users praise the ${topCluster.topic}.`;
        else if (topCluster.sentiment === 'negative') summary = `Significant complaints about ${topCluster.topic}.`;
        else summary = `Users discuss ${topCluster.topic} frequently.`;
    }
...
export const analyzeReviews = functions.https.onCall(async (data, context) => {
