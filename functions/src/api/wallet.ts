...
const checkFraudList = async (userId: string, amount: number) => {
    const db = admin.firestore();
...
export const requestWithdrawalV2 = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "User must be logged in");
    }
    ...
    try {
        let fraudResult: any = { decision: 'approve' };
...
export const simulateCashbackEarned = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "User must be logged in");
    }
...
export const getCurrentLimits = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
