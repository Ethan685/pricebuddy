...
async function seed() {
    ...
    for (const product of SAMPLE_PRODUCTS) {
        ...
        for (const m of selectedMerchants) {
            ...
            if (m.currency === "USD") finalPrice = Math.floor(product.minPrice / 1300 * m.priceMultiplier);
            else if (m.currency === "JPY") finalPrice = Math.floor(product.minPrice / 9 * m.priceMultiplier);
            else finalPrice = Math.floor(product.minPrice * m.priceMultiplier);
