import { BaseMerchantAdapter, Product, PriceInfo, Region } from '../MerchantAdapter';

/**
 * AliExpress Marketplace Adapter
 * Global marketplace from Alibaba Group
 */
export class AliExpressAdapter extends BaseMerchantAdapter {
    readonly name = 'AliExpress';
    readonly region: Region = 'ASIA';
    readonly currency = 'USD';
    readonly countryCode = 'CN';

    private readonly appKey: string;
    private readonly trackingId: string;

    constructor() {
        super();
        this.appKey = process.env.ALIEXPRESS_APP_KEY || '';
        this.trackingId = process.env.ALIEXPRESS_TRACKING_ID || '';
    }

    async search(query: string, limit: number = 10): Promise<Product[]> {
        // Mock data - actual API integration needed
        return [{
            id: 'AE001',
            title: `${query} - AliExpress`,
            price: 249.99,
            currency: 'USD',
            merchantName: this.name,
            region: this.region,
            productUrl: 'https://www.aliexpress.com/item/AE001.html',
            inStock: true,
            rating: 4.7,
            reviewCount: 2345
        }].map(p => ({
            ...p,
            affiliateUrl: this.getAffiliateLink(p.productUrl, p.id)
        }));
    }

    async getPrice(productId: string): Promise<PriceInfo> {
        return {
            productId,
            price: 249.99,
            currency: this.currency,
            priceKRW: 333000,
            lastUpdated: new Date(),
            inStock: true
        };
    }

    getAffiliateLink(productUrl: string, productId?: string): string {
        if (!this.trackingId) return productUrl;

        // AliExpress affiliate link format
        try {
            return `https://s.click.aliexpress.com/e/${this.trackingId}`;
        } catch {
            return productUrl;
        }
    }
}

/**
 * Rakuten (楽天) Marketplace Adapter
 * Japan's largest e-commerce platform
 */
export class RakutenAdapter extends BaseMerchantAdapter {
    readonly name = 'Rakuten';
    readonly region: Region = 'JP';
    readonly currency = 'JPY';
    readonly countryCode = 'JP';

    private readonly affiliateId: string;

    constructor() {
        super();
        this.affiliateId = process.env.RAKUTEN_AFFILIATE_ID || '';
    }

    async search(query: string, limit: number = 10): Promise<Product[]> {
        return [{
            id: 'RAK001',
            title: `${query} - 楽天市場`,
            price: 39800,
            currency: 'JPY',
            merchantName: this.name,
            region: this.region,
            productUrl: 'https://item.rakuten.co.jp/RAK001/',
            inStock: true,
            rating: 4.5,
            reviewCount: 1567
        }].map(p => ({
            ...p,
            affiliateUrl: this.getAffiliateLink(p.productUrl, p.id)
        }));
    }

    async getPrice(productId: string): Promise<PriceInfo> {
        return {
            productId,
            price: 39800,
            currency: this.currency,
            priceKRW: 354000,
            lastUpdated: new Date(),
            inStock: true
        };
    }

    getAffiliateLink(productUrl: string, productId?: string): string {
        if (!this.affiliateId) return productUrl;

        try {
            const url = new URL(productUrl);
            url.searchParams.set('scid', this.affiliateId);
            return url.toString();
        } catch {
            return productUrl;
        }
    }
}

/**
 * Lazada Marketplace Adapter
 * Southeast Asia's leading platform (Alibaba Group)
 */
export class LazadaAdapter extends BaseMerchantAdapter {
    readonly name = 'Lazada';
    readonly region: Region = 'SEA';
    readonly currency = 'USD';
    readonly countryCode = 'SG';

    async search(query: string, limit: number = 10): Promise<Product[]> {
        return [{
            id: 'LAZ001',
            title: `${query} - Lazada`,
            price: 269.99,
            currency: 'USD',
            merchantName: this.name,
            region: this.region,
            productUrl: 'https://www.lazada.sg/products/LAZ001',
            inStock: true,
            rating: 4.4,
            reviewCount: 892
        }].map(p => ({
            ...p,
            affiliateUrl: this.getAffiliateLink(p.productUrl, p.id)
        }));
    }

    async getPrice(productId: string): Promise<PriceInfo> {
        return {
            productId,
            price: 269.99,
            currency: this.currency,
            priceKRW: 359000,
            lastUpdated: new Date(),
            inStock: true
        };
    }

    getAffiliateLink(productUrl: string, productId?: string): string {
        const affiliateId = process.env.LAZADA_AFFILIATE_ID;
        if (!affiliateId) return productUrl;

        return `${productUrl}?aff_short_key=${affiliateId}`;
    }
}
