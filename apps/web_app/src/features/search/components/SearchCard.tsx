          ...
          <div className="mt-auto pt-4 border-t border-border/30">
            <div className="flex items-end justify-between mb-2">
              <div className="flex-1">
                <div className="text-xs text-textMuted mb-1">Min Price</div>
                <div className="text-2xl font-bold text-successNeon font-display">
                  {/* Robust Price Display: KRW > USD/Other > Checking */}
                  {minPrice > 0
                    ? formatKrw(minPrice)
                    : (item.price ? `${item.currency || ""} ${item.price}` : "Checking...")}
                </div>
                {hasPriceRange && (
                  <div className="text-xs text-textMuted mt-1">
                    Max {formatKrw(maxPrice)}
                  </div>
                )}
              </div>
